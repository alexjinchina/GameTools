...
public class SaveFileHelperModule extends ReactContextBaseJavaModule {
    ...
    public SaveFileHelperModule(ReactApplicationContext reactContext) {
        ...
        super(reactContext);setLoggingLevel("info");
    }
    ...
    private WritableMap loadSQLiteTable(SQLiteDatabase db, String tableName, ReadableMap config) {
        ...
        if (config == null || !config.hasKey(DATA)) {
            ...
            FLog.d(TAG, "config `%s` type is %s", DATA, config.getType(DATA).name());
            dataCols = new ArrayList<>();
            String columnName;
            switch (config.getType(DATA)) {
                case String:
                    columnName = config.getString(DATA);
                    FLog.d(TAG, "add data column: %s", columnName);
                    dataCols.add(columnName);
                    break;
                case Map:
                    columnName = getDataColumnName(config.getMap(DATA));
                    FLog.d(TAG, "add data column: %s", columnName);
                    dataCols.add(columnName);
                    break;
                case Array:
                    ...
                    for (int i = 0; i < data.size(); ++i) {
                        switch (data.getType(i)) {
                            ...
                            case Map:
                                ...
                                break;
                            default:
                                throw new RuntimeException(String.format("invalid data column type: %s", data.getType(i)));
                        }
                        FLog.d(TAG, "add data column: %s", columnName);
                        dataCols.add(columnName);
                    }
                    break;
                default:
                    throw (new RuntimeException(String.format("invalid type `%s` of `%s`!",
                            config.getType(DATA).toString(), DATA)));
            }
        }
        ...
        Cursor cursor = db.query(tableName,
                dataCols.toArray(new String[dataCols.size()]),
                null,
                null, null, null, null);
        if (cursor.moveToFirst()) {
            ...
            do {
                ...
                if (dataCols.size() == dataColumnStartIndex + 1) {
                    putSQLiteField(cursor, tableData, key, dataColumnStartIndex + 1);
                } else {
